...
public static class DbSeeder
{
    public static async Task SeedAsync(AppDbContext db)
    {
        ...
        db.RecipeIngredients.AddRange(
            ...
            new RecipeIngredient { RecipeId = burritoBowl.Id, IngredientId = tacoSeasoning.Id, Quantity = 2, Unit = "tbsp" },
        ...
        db.RecipeSteps.AddRange(
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 1, Instruction = "Cook {ingredients} until the beef is browned and seasoned.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 10, IsPassive = false, PrepCategory = PrepStepCategory.CookProtein, LinkedIngredientIds = new List<int> { groundBeef.Id, tacoSeasoning.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 2, Instruction = "Portion {ingredients} as the grain base.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 3, IsPassive = false, PrepCategory = PrepStepCategory.AssemblePortion, LinkedIngredientIds = new List<int> { brownRice.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 3, Instruction = "Drain, rinse, and portion {ingredients}.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 3, IsPassive = false, PrepCategory = PrepStepCategory.AssemblePortion, LinkedIngredientIds = new List<int> { blackBeans.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 4, Instruction = "Thaw or drain, then portion {ingredients} without warming it.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 3, IsPassive = false, PrepCategory = PrepStepCategory.AssemblePortion, LinkedIngredientIds = new List<int> { corn.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 5, Instruction = "Pack {ingredients} separately or spoon it into each bowl.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 2, IsPassive = false, PrepCategory = PrepStepCategory.MixSauce, LinkedIngredientIds = new List<int> { salsa.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 6, Instruction = "Slice selected pepper add-ins and store separately: {ingredients}.", TimingTag = TimingTag.PrepAhead, DurationMinutes = 6, IsPassive = false, PrepCategory = PrepStepCategory.WashChop, LinkedIngredientIds = new List<int> { bellPepper.Id }, ScaleByLinkedIngredients = true },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 7, Instruction = "Reheat the prepared bowl base if you want it warm.", TimingTag = TimingTag.DayOfActive, DurationMinutes = 3, IsPassive = false, PrepCategory = PrepStepCategory.FreshFinish, LinkedIngredientIds = new List<int> { brownRice.Id, blackBeans.Id, groundBeef.Id, corn.Id } },
            new RecipeStep { RecipeId = burritoBowl.Id, StepNumber = 8, Instruction = "Finish with any fresh toppings you selected: {ingredients}.", TimingTag = TimingTag.DayOfActive, DurationMinutes = 2, IsPassive = false, PrepCategory = PrepStepCategory.FreshFinish, LinkedIngredientIds = new List<int> { avocado.Id, cheddarCheese.Id, picoDeGallo.Id, romaineLettuce.Id, sourCream.Id }, ScaleByLinkedIngredients = true }
        );
